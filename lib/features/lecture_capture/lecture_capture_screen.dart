...
class _LectureCaptureScreenState extends ConsumerState<LectureCaptureScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final notesState =
        ref.watch(notesGeneratorProvider(widget.courseId));
    final notesNotifier =
        ref.read(notesGeneratorProvider(widget.courseId).notifier);

    return Scaffold(
      appBar: AppBar(
        title: const Text('Lecture Capture'),
        actions: [
          // Delete notes button — only visible when notes are displayed.
          if (notesState.notes != null)
            IconButton(
              icon: const Icon(Icons.delete_outline),
              tooltip: 'Clear notes',
              onPressed: () => _confirmDeleteNotes(context, notesNotifier),
            ),
        ],
      ),
      body: notesState.isLoadingHistory
  ...
  void _confirmDeleteNotes(
      BuildContext context, NotesGeneratorNotifier notifier) {
    showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Clear Notes'),
        content: const Text(
          'Delete these lecture notes? This cannot be undone.',
        ),
        actions: [
          TextButton(
      ...
      if (confirmed == true) notifier.deleteCurrentNotes();
    });
  }
}
